...
private:
    ...
    //the 3th layer's out   out
    struct pBox *conv3;
    struct pBox *conv4_matrix;
    //the 4th layer's out   out
	struct pBox *conv4;
	//the 5_1th layer's out out
	struct pBox *score_matrix;
    struct pBox *score_;
    //the 5_2th layer's out  out
    ...
    struct Weight *conv5c1_wb;
    struct Weight *conv5c2_wb;
...
private:
	...
	struct pBox *conv5_matrix;
	struct pBox *conv5_out;
	...
	struct Weight *conv5_wb;
	struct pRelu *prelu_gmma5;
    struct Weight *fc6_wb;
    struct pRelu *prelu_gmma6;
    struct Weight *score_wb;
    struct Weight *location_wb;
...
{
public:
    mtcnn(int row, int col, int minsize = 72);
    ~mtcnn();

	//这里默认是提取关键点的，如果你的模型没有关键点也没关系，影响是没有的。如果有关键点，那么就提取
	vector<Rect> detectObject(Mat &image);
private:
    Mat reImage;
