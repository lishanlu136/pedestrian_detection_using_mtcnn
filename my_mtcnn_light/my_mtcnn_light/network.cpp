...
void image2Matrix(const Mat &image, const struct pBox *pbox){
    ...
    for (int rowI = 0; rowI < image.rows; rowI++){
        for (int colK = 0; colK < image.cols; colK++){
            *p = (image.at<Vec3b>(rowI, colK)[0] - 127.5)*0.0078125;   //数据层减均值并归一化
...
void featurePadInit(const pBox *pbox, pBox *outpBox, const int padW, const int padH){
    if ((padW <= 0)&&(padH <= 0)){
...
void featurePad(const pBox *pbox, const pBox *outpBox, const int padW, const int padH){
...
void feature2MatrixInit(const pBox *pbox, pBox *Matrix, const Weight *weight){
    ...
    int kernelWidth = weight->kernelWidth;
	int kernelHeight = weight->kernelHeight;
	int strideW = weight->strideW;
    int strideH = weight->strideH;
	int padW = weight->padW;
	int padH = weight->padH;
    int w_out = (pbox->width - kernelWidth + 2 * padW) / strideW + 1; //Õâ¸ö¹«Ê½Ò»¶¨Òª¸ãÇå³þ£¬¿ÉÒÔ×Ô¼ºÈ¥»­¸ö¾ØÕó¿´¿´
    int h_out = (pbox->height - kernelHeight + 2 * padH) / strideH + 1;
    Matrix->width = pbox->channel*kernelWidth*kernelHeight;     //Î´×ªÖÃÇ°µÄ¿í¶È
...
void feature2Matrix(const pBox *pbox, pBox *Matrix, const Weight *weight){
	...
	int padW = weight->padW;
	int padH = weight->padH;
	int w_out = (pbox->width - kernelWidth + 2 * padW) / strideW + 1;  
	int h_out = (pbox->height - kernelHeight + 2 * padH) / strideH + 1;
	if ((padW == 0) && (padH == 0))
	{
		...
		for (int row = 0; row< h_out; row++){
			for (int col = 0; col < w_out; col++){
				pIn = pbox->pdata + row*strideH*pbox->width + col*strideW;

				for (int channel = 0; channel < pbox->channel; channel++){
					ptemp = pIn + channel*pbox->height*pbox->width;
					for (int kernelRow = 0; kernelRow < kernelHeight; kernelRow++){
						memcpy(p, ptemp, kernelWidth*sizeof(mydataFmt));
						p += kernelWidth;
						ptemp += pbox->width;
					}
				}
	...
	else
	{
		...
		featurePadInit(pbox, padpbox, padW, padH);
		featurePad(pbox, padpbox, padW, padH);
		mydataFmt *p = Matrix->pdata;
		mydataFmt *pIn;
		mydataFmt * ptemp;
		for (int row = 0; row< h_out; row++){
			for (int col = 0; col < w_out; col++){
				pIn = padpbox->pdata + row*strideH*padpbox->width + col*strideW;

				for (int channel = 0; channel < padpbox->channel; channel++){
					ptemp = pIn + channel*padpbox->height*padpbox->width;
					for (int kernelRow = 0; kernelRow < kernelHeight; kernelRow++){
						memcpy(p, ptemp, kernelWidth*sizeof(mydataFmt));
						p += kernelWidth;
						ptemp += padpbox->width;
...
void convolutionInit(const Weight *weight, const pBox *pbox, pBox *outpBox, const struct pBox *matrix){
    ...
    outpBox->width = (pbox->width - weight->kernelWidth + 2 * weight->padW) / weight->strideW + 1;
    outpBox->height = (pbox->height - weight->kernelHeight + 2 * weight->padH) / weight->strideH + 1;
    outpBox->pdata = (mydataFmt *)malloc(weight->selfChannel*matrix->height*sizeof(mydataFmt));    //??? matrix->height = w_out*h_out;
...
void convolution(const Weight *weight, const pBox *pbox, pBox *outpBox,  const struct pBox *matrix){
    ...
    //if((weight->padW==0)&&(weight->padH==0)){
    	//C←αAB + βC
   ...
   // else{
  //      struct pBox *padpbox = new pBox;
		//featurePadInit(pbox, padpbox, weight->padW, weight->padH);
		//featurePad(pbox, padpbox, weight->padW, weight->padH);
		//memset(matrix->pdata, 0, matrix->channel*matrix->width*matrix->height*sizeof(mydataFmt));
		//feature2Matrix(padpbox, matrix, weight);
  //      //C←αAB + βC
  //      //                1              2            3              4     C's size    5              k     alpha     A*              A'col             B*           B'col    beta      C*           C'col
  //      cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans, weight->selfChannel, matrix->height, matrix->width, 1, weight->pdata, matrix->width, matrix->pdata, matrix->width, 0, outpBox->pdata, matrix->height);
  //      freepBox(padpbox);
...
long initConvAndFc(struct Weight *weight, int schannel, int lchannel,
    ...
    weight->kernelWidth = kernelW;
	weight->kernelHeight = kernelH;
	weight->strideW = strideW;
    weight->strideH = strideH;
	weight->padW = padW;
    weight->padH = padH;
    ...
    long byteLenght = weight->selfChannel*weight->lastChannel*weight->kernelWidth*weight->kernelHeight;
